...
{
    free(data->key); //we need to free the keyword as well
    free(data);
...
void inorder_max(node *x,rb_tree* tree){
    if (x != NIL){
        inorder_max(x->left,tree);
	if(node_max == NULL || x->data->num_times > node_max->num_times){
		node_max = x->data;
	}
        inorder_max(x->right,tree);
    }
}

void find_ncoincidences_word(rb_tree *tree, int opt, char * word){
	if(opt == 0){
		inorder_max(tree->root,tree);
		printf("Key: %s | Times: %d\n", node_max->key, node_max->num_times);
	}else if (opt == 1){
		node_data *aux;
		aux = find_node(tree, word);
		if (aux != NULL) {
			printf("Key: %s | Times: %d\n", aux->key, aux->num_times);
		}else printf("\nThe word %s is not in the tree\n",word);
	}
}
...
void inorder_save(node *x,FILE *fp){
    if (x != NIL){
        inorder_save(x->left,fp);
        
        int size_word = strlen(x->data->key);
        fwrite(&size_word, sizeof(int), 1, fp);
        fwrite(x->data->key, 1, size_word,fp);
        fwrite(&x->data->num_times, sizeof(int), 1, fp);
        
        inorder_save(x->right,fp);
    }
}

void save_tree_disk(rb_tree *tree, FILE *fp){
    if(tree->root!=NIL){
        fwrite(&tree->size, 1, sizeof(int), fp);
        inorder_save(tree->root, fp);
    }
}
